  ...
  return (
    <main className="min-h-screen">
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14">
        <div className="max-w-4xl">
          <div className="animate-fade-in-up">
            <div className="inline-flex max-w-full items-center gap-3 rounded-full border border-border bg-white/90 px-4 py-2 shadow-sm backdrop-blur-sm sm:px-5">
              ...
              <p className="truncate text-[10px] font-black uppercase tracking-[0.25em] text-accent sm:text-xs sm:tracking-[0.35em]">
                Medidas y cuidados
              </p>
            </div>
          </div>

          <h1 className="mt-7 animate-fade-in-up animate-delay-100 text-3xl font-black leading-tight text-[#1e1e1e] sm:mt-8 sm:text-4xl md:text-5xl">
          ...
          <p className="mt-5 animate-fade-in-up animate-delay-200 max-w-2xl text-sm leading-7 text-muted sm:mt-6 sm:text-base sm:leading-8">
            Una correcta selección de talla y cuidado de lavado ayuda a mantener la
            presentación, color, forma y durabilidad de cada prenda.
          </p>
        </div>

        <div className="mt-10 grid animate-fade-in-up animate-delay-200 gap-6 lg:mt-12 lg:grid-cols-[1.2fr_0.8fr] lg:gap-8">
          <div className="rounded-3xl border border-border bg-white/80 p-4 shadow-sm backdrop-blur-sm sm:p-6">
            ...
            <div className="mt-6 grid gap-3 md:hidden">
              {sizeRows.map((row) => (
                <div
                  key={row[0]}
                  className="rounded-2xl border border-border bg-surface-2/70 p-4 transition-colors hover:bg-accent-soft/50"
                >
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <span className="text-xs font-black uppercase tracking-[0.2em] text-muted">
                      Talla
                    </span>
                    <span className="rounded-full bg-white px-4 py-1.5 text-lg font-black text-[#1e1e1e] shadow-sm">
                      {row[0]}
                    </span>
                  </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[640px] text-left text-sm">
                  <thead className="bg-surface-2 text-xs uppercase tracking-[0.2em] text-muted">
                    <tr>
                      <th className="whitespace-nowrap px-5 py-4">Talla</th>
                      <th className="whitespace-nowrap px-5 py-4">Pecho</th>
                      <th className="whitespace-nowrap px-5 py-4">Cadera</th>
                  ...
                  <tbody className="divide-y divide-border bg-white/70">
                    {sizeRows.map((row) => (
                      ...
                      >
                        <td className="whitespace-nowrap px-5 py-4 font-black text-[#1e1e1e]">
                          {row[0]}
                        </td>
          ...
          <aside className="rounded-3xl border border-border bg-white/80 p-4 shadow-sm backdrop-blur-sm sm:p-6">
            <h2 className="text-xl font-black text-[#1e1e1e] sm:text-2xl">
              Cuidados de prenda
            </h2>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
              {careItems.map((item) => (
                <div
                  key={item.label}
                  className="group flex items-start gap-3 rounded-2xl bg-surface-2/80 px-4 py-3 transition-colors hover:bg-accent-soft/80"
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-bold leading-5 text-[#1e1e1e]">
                      {item.label}
                    </p>
        ...
        <div className="mt-8 grid animate-fade-in-up animate-delay-300 gap-4 sm:grid-cols-2 lg:grid-cols-4 lg:gap-6">
          {tips.map((t) => (
            <div
              key={t.title}
              className="rounded-3xl border border-border bg-white/80 p-5 shadow-sm backdrop-blur-sm transition-all hover:-translate-y-0.5 hover:border-accent hover:shadow-md sm:p-6"
            >
