...
function Icon({ type }: { type: AdminTab }) {
  ...
  return <div className="h-5 w-5 shrink-0">{icons[type]}</div>;
}
      ...
      <div className="flex min-h-screen">
        ...
        <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 shrink-0 flex-col border-r border-accent-soft/50 bg-[#ece5dc] backface-hidden lg:flex">
          ...
          <div className="border-t border-accent-soft/50 p-4 bg-[#ece5dc]/50">
            <button
              onClick={onLogout}
              className="flex w-full items-center justify-center gap-2 rounded-xl border border-red-200 bg-red-50/60 px-4 py-2.5 text-sm font-medium text-slate-700 transition-all duration-200 hover:bg-red-500 hover:text-white hover:shadow-md hover:shadow-red-500/10 active:scale-[0.98]"
            >
